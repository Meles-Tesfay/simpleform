...
export default function App(){
  ...
  async function submit(){
    try{
      const res = await fetch('/api/survey',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(answers)})
      if(res.ok){ setI(stepCount); return }
      ...
      <div className="card">
        ...
        <div className="question-area">
          <div className="question enter">
            ...
            <div className="small-note">Almost done 👀 — quick and anonymous.</div>
            <button className="btn primary" style={{marginTop:16}} onClick={()=>setI(0)}>Start Survey</button>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="app">
      <div className="card">
        <div className="meta">
          <div className="progress"><span style={{width:`${Math.round((i/(stepCount))*100)}%`}}/></div>
          ...
          <div className="question enter">
            ...
            {q.type==='single' && (
              <div className="options">
                {q.options.map(opt=> (
                  <button key={opt} className={`option-button`} onClick={()=>{ setAnswer(q.id,opt); setI(s=>s+1) }}>{opt}</button>
                ))}
              </div>
            )}
            ...
            {q.type==='composite' && (
              <div>
                <div className="options">
                  {['< $1','$1 - $3','$3 - $6','> $6'].map(r=> (
                    <button key={r} className='option-button' onClick={()=>{ setAnswer('spend_per_visit',r); setI(s=>s+1) }}>{r}</button>
                  ))}
                </div>
                <div style={{marginTop:14}}>
                  <div className='small-note'>Or drag to pick an approximate amount (optional)</div>
                  <input className='slider' type='range' min='0' max='100' onInput={e=>setAnswer('spend_slider',e.target.value)} />
                  <input className='input' placeholder='Weekly total spending (optional)' type='number' onChange={e=>setAnswer('weekly_total',e.target.value)} />
                </div>
              </div>
            )}

            {q.type==='business' && (
              <BusinessInput onChoose={(v)=>{ setAnswer('business_name',v) }} onSetFreq={(v)=>{ setAnswer('business_freq',v); setI(s=>s+1)}} />
            )}
        ...
        <div className='controls'>
          <button className='btn ghost' onClick={()=>setI(s=>Math.max(-1,s-1))} style={{visibility:i===0?'hidden':'visible'}}>Back</button>
          <button className='btn primary' onClick={()=>{
            // if current is multi require selection
            if(q.type==='multi'){ if(!(answers[q.id]||[]).length){ alert('Please select at least one option'); return } }
            if(i===questions.length-1){ submit() } else { setI(s=>s+1) }
          }}>{i===questions.length-1?'Finish':'Next'}</button>
        </div>
      </div>
    </div>
  )
...
function MultiSelect({q,value,onChange,onNext}){
  ...
  return (
    <div>
      <div className='options'>
        {q.options.map(opt=> (
          <button key={opt} className={`option-button ${sel.has(opt)?'selected':''}`} onClick={()=>toggle(opt)}>{opt}</button>
        ))}
      </div>
      {sel.has('Other') && <input className='input' placeholder='If Other, tell us which (optional)' onChange={e=>onChange([...(Array.from(sel).filter(x=>x!=='Other')), e.target.value])} />}
...
function BusinessInput({onChoose,onSetFreq}){
  ...
  return (
    <div>
      <div className='chips'>
        {suggestions.map(s=> <div key={s} className='chip' onClick={()=>{ setCustom(''); onChoose(s) }}>{s}</div>)}
      </div>
      ...
      <div style={{marginTop:14}}>
        ...
        {['Daily','Weekly','Occasionally'].map(f=> <button key={f} className='option-button' style={{marginTop:8}} onClick={()=>onSetFreq(f)}>{f}</button>)}
      </div>
    </div>
  )
}
